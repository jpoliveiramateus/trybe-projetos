...
const createMenu = (object) => {
  const result = {
    fetchMenu: () => object,
    consumption: [],
    order: (food, objetoRetornado) => objetoRetornado.consumption.push(food),
    pay: (objetoRetornado) => {
      const consume = objetoRetornado.consumption;
      let pay = 0;
      for (let i = 0; i < consume.length; i += 1) {
        pay += Object.values(objetoRetornado.fetchMenu())[0][consume[i]];
      }
      return pay;
    },
  };
